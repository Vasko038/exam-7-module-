...
import { addItem, deleteItem, updateItem } from "../store/slices/Page2Slice";
...
function Page1() {
  const TableData = useSelector((state: RootState) => state.page2.data);
  ...
  return (
    <div className="p-[20px]">
      ...
      <div className="flex flex-col p-[10px] rounded-lg mt-">
        <Typography.Title rootClassName="mb-5" level={4} className="block">
          Table
        </Typography.Title>
        <div>
          <div className="grid items-center grid-cols-9 gap-4 px-3 py-4 mb-3 bg-white rounded-lg shadow-lg">
      ...
      >
        <Form layout="vertical" form={form} onFinish={onFinish}>
          ...
          <Form.Item
            ...
            rules={[{ required: true, message: "Please input!" }]}
          >
            <Input placeholder="Enter telegram"></Input>
          </Form.Item>
          <Form.Item
            label="Instagram"
            name={"instagram"}
            rules={[{ required: true, message: "Please input!" }]}
          >
            <Input placeholder="Enter Instagram"></Input>
          </Form.Item>
          <Button
            htmlType="submit"
            type="primary"
